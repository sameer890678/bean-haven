import Image from "next/image";

export default function FeaturedMenu() {
  return (
    <section id="menu" className="bg-amber-100 py-16 lg:py-28 px-6 md:px-12 lg:px-20">

      {/* Heading */}
      <div className="text-center mb-14">
        <h2 className="text-4xl md:text-5xl font-bold text-amber-900 tracking-tight">
          Featured Menu
        </h2>

        <p className="mt-5 max-w-3xl mx-auto text-base md:text-lg leading-8 text-stone-600 px-2">
          Explore our handpicked selection of handcrafted coffee, prepared with
          premium beans and perfected by our experienced baristas. Every cup is
          made to deliver comfort, warmth, and unforgettable flavor.
        </p>
      </div>

      {/* Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">

        {/* Card 1 */}
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 w-full max-w-sm mx-auto">

          <Image
            src="/images/Cappuccino.jpg"
            alt="Cappuccino"
            width={450}
            height={300}
            className="w-full h-56 md:h-60 object-cover rounded-xl"
          />

          <div className="p-6">

            <h3 className="text-2xl font-bold text-amber-900">
              Cappuccino
            </h3>

            <p className="mt-3 text-stone-600 leading-7 text-justify">
              A rich espresso blended with silky steamed milk and finished with
              a thick layer of creamy foam.
            </p>

            <div className="mt-6 flex items-center justify-between">

              <span className="text-2xl font-bold text-amber-700">
                $3.50
              </span>

            </div>

          </div>

        </div>

        {/* Card 2 */}
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 w-full max-w-sm mx-auto">

          <Image
            src="/images/espresso.jpg"
            alt="Espresso"
            width={450}
            height={300}
            className="w-full h-56 md:h-60 object-cover rounded-xl"
          />

          <div className="p-6">

            <h3 className="text-2xl font-bold text-amber-900">
              Espresso
            </h3>

            <p className="mt-3 text-stone-600 leading-7 text-justify">
              A bold and concentrated coffee with an intense aroma, perfect for
              those who enjoy strong flavors.
            </p>

            <div className="mt-6 flex items-center justify-between">

              <span className="text-2xl font-bold text-amber-700">
                $2.50
              </span>
            </div>

          </div>

        </div>

        {/* Card 3 */}
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 w-full max-w-sm mx-auto">

          <Image
            src="/images/latte.jpg"
            alt="Latte"
            width={450}
            height={300}
            className="w-full h-56 md:h-60 object-cover rounded-xl"
          />

          <div className="p-6">

            <h3 className="text-2xl font-bold text-amber-900">
              Latte
            </h3>

            <p className="mt-3 text-stone-600 leading-7 text-justify">
              Smooth espresso combined with perfectly steamed milk to create a
              creamy and comforting classic.
            </p>

            <div className="mt-6 flex items-center justify-between">

              <span className="text-2xl font-bold text-amber-700">
                $5.00
              </span>
              
            </div>

          </div>

        </div>

      </div>

    </section>
  );
}

